...
def test_find_suitable_user():
    ...
    # Поиск пользователя с именем "Olga" с помощью генератора списка
    suitable_users = [user for user in users if user["name"] == "Olga"]
    # Поиск пользователя с именем "Olga" с помощью функции filter
...
def print_readable_function_name(func, **kwargs):
...
def open_browser(browser_name):
    actual_result = print_readable_function_name(open_browser, browser_name=browser_name)
    assert actual_result == "Open Browser [Chrome]"


def go_to_companyname_homepage(page_url):
    actual_result = print_readable_function_name(go_to_companyname_homepage, page_url=page_url)
    assert actual_result == "Go To Companyname Homepage [https://companyname.com]"


def find_registration_button_on_login_page(page_url, button_text):
    actual_result = print_readable_function_name(find_registration_button_on_login_page,
